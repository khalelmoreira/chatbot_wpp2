...
- Prompt text lives in `src/models/prompts/`, one file per concern (`prestador_prompts.py`, `consulta_prompts.py`, `nfse_prompts.py`, `conversation_prompts.py`), not a flat `prompts/` at `src/` root. Shared fragments (role lines, anti-invention rule, lay-term glossary, `ARG` runtime placeholder) live in `_common.py` and are composed into each prompt via f-string.
