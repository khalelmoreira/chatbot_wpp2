...
- [✓] **Greater message history for AI context:** `classify` and `respond` calls now receive the last ~8 turns (`MsgManager.get_ai_history` → `AIOperations` → `AIClient`), so a bare "sim" after the bot's question classifies correctly instead of looping. Extraction stays message-local. Incoming USER messages are persisted once centrally in `initial_handler` (all flows), not just the ACTIVE path.
