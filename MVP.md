...
- [ ] **ISS rates — real resolution (blocked on partners):** the recent RJ ISS law change is not yet confirmed by the partners, so the real ADN sync (`iss_rate_sync.py`) can't be trusted. Stopgap for E2E testing: `python -m src.workers.iss_rate_seed` populates `iss_rates` with a flat 5.0% for every national code so `IssResolutionService`/`_iss_ok()` run the production path against a table that always answers (`--clear` to undo; seed rows use `vigencia_inicio = 1900-01-01` so real sync rows supersede them). Before launch: (a) get the confirmed rates from partners, (b) run the real `iss_rate_sync` against the ADN cert, (c) close the `collecting_service.py` TODO so `resolved.aliquota` actually propagates to `TomadorManager` instead of the hardcoded `ALIQUOTA_ISS = 5.0`, (d) clear the seed rows.
